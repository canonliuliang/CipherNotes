...
- 不想管理员变成万能钥匙：管理员负责账号和备份秩序，普通用户内容仍由自己的密码和恢复码保护。
...
- 多账号：管理员负责账号与数据生命周期，普通用户内容由自己的密码和恢复码保护。
...
- 管理员密码用于批准注册新用户、管理用户、备份还原和升级旧保险库，不能直接查看普通用户的笔记。
...
- 用户管理新增修改管理员密码、删除用户和清空全部数据并重新开始。
- 新增安全中心。
