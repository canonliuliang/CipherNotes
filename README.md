...
- Allows standard macOS screenshots, screen recording, and meeting-window sharing.
- Keeps Highest Protection focus shielding, automatic locking, and sensitive preview-cache clearing.
- Clarifies in the privacy disclosure that visible content can be captured by the user or operating system.
- Leaves the restored 1.0.8 interface and current login page unchanged.

### 1.1.6 - 经典界面回归

- Restores the complete 1.0.8 post-login interface for notes, vault, viewer, toolbar, settings, and Security Center.
- Keeps the current responsive login, registration, and recovery page unchanged.
- Retains the current encrypted data format, equal-account security model, and later security fixes.
- Adds only the compatibility changes required by current asynchronous preview and import APIs.

### 1.1.5 - 主工作区恢复

- Removes the global bottom utility strip after login so the notes and vault workspace keeps its full height.
- Keeps Security Center visible as a dedicated native toolbar button.
- Returns secondary utilities to the standard macOS menu bar instead of crowding the workspace.
- Adds an unlocked minimum-window render regression test.
